...
// Fetch users from the API along with total_users
async function fetchUsers(page: number, perPage: number): Promise<{ totalUsers: number, users: User[] }> {
  try {
...
// Pagination
function initPagination(totalUsers: number, currentPage: number, perPage: number) {
  ...
  if (nav) {
    ...
    `;
    // Click event listener for prevButton
    prevButton.addEventListener('click', () => {
      if (currentPage > 1) {
        fetchUsers(currentPage - 1, perPage).then(response => {
          const totalUsers = response.totalUsers;
          const users = response.users;
          displayUsers(users); // Pass the users array to displayUsers
          initPagination(totalUsers, currentPage - 1, perPage); // Update pagination
        });
      }
    });
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      // Click event listener for pageButton
      pageButton.addEventListener('click', () => {
        fetchUsers(i, perPage).then(response => {
          const users = response.users;
          displayUsers(users); // Pass the users array to displayUsers
          initPagination(response.totalUsers, i, perPage); // Update pagination
        });
      });
    ...
    `;
    // Click event listener for nextButton
